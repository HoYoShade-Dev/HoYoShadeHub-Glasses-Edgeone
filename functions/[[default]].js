...
(function() {
    ...
    // Get original URL
    // If the current URL is https://proxy.com/https://target.com/path
    // originalUrlStr should be https://target.com/path
    let originalUrlStr = window.location.href.substring(proxyPrefix.length);
    
    // Handle cases where the URL might be malformed or double proxied
    if (originalUrlStr.startsWith(proxyPrefix)) {
        originalUrlStr = originalUrlStr.substring(proxyPrefix.length);
    }
    
    // Parse the original URL for location object simulation
    let originalUrl;
    try {
    ...
    const originalHostWithSchema = originalUrl.protocol + '//' + originalUrl.host + '/';
    ...
    function changeURL(url) {
        ...
        // Skip already proxied URLs (simple check)
        if (urlStr.startsWith(proxyPrefix + 'http://') || 
            urlStr.startsWith(proxyPrefix + 'https://')) {
            return urlStr;
        }
        
        // Remove proxy prefix if present in the input (to get the raw target URL)
        if (urlStr.startsWith(proxyPrefix)) {
            urlStr = urlStr.substring(proxyPrefix.length);
        }
        if (urlStr.startsWith(proxyHost + '/')) {
            urlStr = urlStr.substring(proxyHost.length + 1);
        }
        if (urlStr.startsWith(proxyHost)) {
            urlStr = urlStr.substring(proxyHost.length);
        }
        
        try {
            // Resolve to absolute URL against the *original* target URL
            // This is crucial: relative paths must be resolved against the target, not the proxy
            let absoluteUrl = new URL(urlStr, originalUrlStr).href;
            
            // Replace any proxy host references with original host
            absoluteUrl = absoluteUrl.replaceAll(window.location.href, originalUrlStr);
            absoluteUrl = absoluteUrl.replaceAll(encodeURI(window.location.href), encodeURI(originalUrlStr));
            absoluteUrl = absoluteUrl.replaceAll(encodeURIComponent(window.location.href), encodeURIComponent(originalUrlStr));
            absoluteUrl = absoluteUrl.replaceAll(proxyHost, originalHost);
            absoluteUrl = absoluteUrl.replaceAll(encodeURI(proxyHost), encodeURI(originalHost));
            absoluteUrl = absoluteUrl.replaceAll(encodeURIComponent(proxyHost), encodeURIComponent(originalHost));
            
            // Fix double proxying if the resolved URL somehow includes the proxy prefix again
            if (absoluteUrl.startsWith(proxyPrefix)) {
                absoluteUrl = absoluteUrl.substring(proxyPrefix.length);
            }

            // Add proxy prefix
            return pathAfterAdd + proxyPrefix + absoluteUrl;
        } catch (e) {
    ...
    function getOriginalUrl(url) {
        ...
        if (str.startsWith(proxyPrefix)) {
            return str.substring(proxyPrefix.length);
        }
        return str;
    }
    
    // ===== ProxyLocation class to simulate the original location object =====
    class ProxyLocation {
        ...
        get protocol() {
            return originalUrl.protocol;
        }
        
        set protocol(value) {
            originalUrl.protocol = value;
            this.originalLocation.href = proxyPrefix + originalUrl.href;
        }
    ...
    // ===== Inject document.location and document.URL =====
    try {
    ...
    try {
        Object.defineProperty(document, '__yproxy_location__', {
            get: function() {
    ...
    // ===== Inject window.__yproxy_location__ =====
    try {
        Object.defineProperty(window, '__yproxy_location__', {
            get: function() {
                return proxyLocation;
            },
            set: function(url) {
    ...
    History.prototype.pushState = function(state, title, url) {
        if (!url) return originalPushState.apply(this, [state, title, url]);
        
        let urlStr = url.toString();
        
        // Handle special cases like DuckDuckGo
        if (urlStr.startsWith('/' + originalUrl.href)) {
            urlStr = urlStr.substring(('/' + originalUrl.href).length);
        }
        if (urlStr.startsWith('/' + originalUrl.href.slice(0, -1))) {
            urlStr = urlStr.substring(('/' + originalUrl.href).length - 1);
        }
        
        const u = changeURL(urlStr);
        return originalPushState.apply(this, [state, title, u]);
    };
    
    History.prototype.replaceState = function(state, title, url) {
        if (!url) return originalReplaceState.apply(this, [state, title, url]);
        ...
        // Handle ipinfo.io style patches
        if (urlStr.startsWith('/' + originalUrl.href.replace('://', ':/'))) {
            urlStr = urlStr.substring(('/' + originalUrl.href.replace('://', ':/')).length);
        }
    ...
    // ===== Hook appendChild (for dynamically created scripts/images) =====
    ...
    // This intercepts direct assignments like img.src = "..."
    const elementPrototypes = [
        ...
        [HTMLInputElement, "src"], // for type="image"
    ];
    ...
    for (const [elementClass, property] of elementPrototypes) {
        ...
        Object.defineProperty(elementClass.prototype, property, {
            get: function() {
                // Return the original URL to the script so it doesn't get confused
    ...
    // ===== MutationObserver to catch innerHTML changes =====
        ...
        mutations.forEach(mutation => {
            mutation.addedNodes.forEach(node => {
                if (node.nodeType === 1) { // ELEMENT_NODE
                    ...
                    // If the node itself has attributes
                    if (node.hasAttribute && node.hasAttribute('src')) node.setAttribute('src', node.getAttribute('src'));
                    if (node.hasAttribute && node.hasAttribute('href')) node.setAttribute('href', node.getAttribute('href'));
                    
                    // And its children
                    if (node.querySelectorAll) {
                        node.querySelectorAll('[href], [src], [integrity]').forEach(el => {
    ...
    console.log('Proxy client hooks installed (Enhanced with location simulation)');
})();
</script>
`;
